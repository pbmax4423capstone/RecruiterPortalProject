  ...
  try {
    ...
    chrome.tabs.sendMessage(tab.id, { action: 'getContactData' }, (data) => {
      if (chrome.runtime.lastError) {
        ...
        elements.previewSubtitle.textContent = 'Try manual entry or navigate to a profile page';
        return;
      }
...
function updatePreview(data) {
  const fullName = [data.firstName, data.lastName].filter(Boolean).join(' ') || 'Unknown Contact';
...
async function checkAuthStatus() {
  const data = await chrome.storage.local.get(['accessToken', 'instanceUrl', 'userName']);
...
async function handleCreateCandidate() {
  ...
  try {
    ...
    const candidateData = {
      ...
      Position_Applying_For__c: elements.position.value || CONFIG.defaults.position,
      Agency__c: elements.agency.value || CONFIG.defaults.agency,
      Assigned_Recruiter__c: CONFIG.testRecruiterId
    };

    // Add optional fields
    if (elements.email.value) candidateData.Email__c = elements.email.value.trim();
    if (elements.phone.value) candidateData.Phone__c = elements.phone.value.trim();
    if (elements.company.value) candidateData.Current_Employer__c = elements.company.value.trim();
    if (elements.headline.value) candidateData.Current_Title__c = elements.headline.value.trim();
    if (elements.location.value) candidateData.Location__c = elements.location.value.trim();
