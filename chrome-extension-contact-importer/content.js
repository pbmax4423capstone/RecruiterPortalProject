...
(function() {
  ...
  function extractZoomInfoData() {
    ...
    try {
      ...
      // Title/Headline
      const titleElement = document.querySelector('[data-testid="profile-title"]') ||
      ...
      // Company
      const companyElement = document.querySelector('[data-testid="profile-company"]') ||
      ...
      if (emailElement) {
        data.email = emailElement.textContent.trim() || emailElement.href.replace('mailto:', '');
      }

      // Phone (if visible)
      const phoneElement = document.querySelector('[data-testid="phone"]') ||
                           document.querySelector('a[href^="tel:"]');
      if (phoneElement) {
        data.phone = phoneElement.textContent.trim() || phoneElement.href.replace('tel:', '');
      }
  ...
  function extractGenericData() {
    ...
    try {
      ...
      const namePatterns = [
        // Schema.org Person
        () => {
          const personSchema = document.querySelector('[itemtype*="schema.org/Person"] [itemprop="name"]');
        ...
        () => {
          const aboutSection = document.querySelector('.about-name, .profile-name, .author-name, [class*="person-name"]');
        ...
        () => {
          ...
          if (words.length >= 2 && words.length <= 4) {
            const commonWords = ['home', 'page', 'profile', 'about', 'contact', 'welcome', 'the', 'and', 'company'];
            const isName = !words.some(w => commonWords.includes(w.toLowerCase()));
      ...
      const companyPatterns = [
        document.querySelector('[itemtype*="schema.org/Organization"] [itemprop="name"]'),
        document.querySelector('.company-name, .organization-name, [class*="company"]'),
