...
const CONFIG = {
  ...
  clientId:
    "3MVG9p1Q1BCe9GmCK.2GT3FP11uBOviHb5qJPJvlkCtUE52LJ_ODY9s2ea_B5E3Fw3xVeiYH3bxLOESNXIMHF", // Production Consumer Key
...
console.log(
  "[CONFIG] Alternative format:",
  `https://${chrome.runtime.id}.chromiumapp.org/oauth2`
);
  ...
  if (stored.accessToken && stored.instanceUrl) {
    ...
    try {
      const response = await fetch(
        `${instanceUrl}/services/data/v59.0/sobjects/User/${currentUserId}`,
        {
    ...
    try {
      ...
      if (response && (response.firstName || response.profileUrl)) {
        ...
        if (!response.firstName && response.profileUrl) {
          ...
          showStatus(
            "Profile URL captured. Please enter name manually.",
            "info"
          );
        }
      } else {
        ...
        showStatus(
          "Could not auto-detect profile. Please enter name manually.",
          "info"
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (responseUrl.includes("error=")) {
        ...
        const errorDesc = errorDescMatch
          ...
          : "";
        throw new Error(
          `OAuth Error: ${errorMsg}${errorDesc ? " - " + errorDesc : ""}`
        );
      }
      ...
      if (
        authError.message &&
        authError.message.includes("Authorization page could not be loaded")
      ) {
        throw new Error(
          "Cannot load Salesforce login page. Please check:\n1. Connected App callback URL includes: " +
            CONFIG.redirectUri +
...
async function handleCreateCandidate() {
  ...
  createBtn.innerHTML =
    '<span class="loading"><span class="spinner"></span> Creating...</span>';
  ...
  try {
    ...
    if (!stored.accessToken) {
      throw new Error(
        "Not authenticated. Please click the extension icon to connect to Salesforce first."
      );
    }
    ...
    if (
      ...
      showStatus(
        "⚠️ This person already exists in Salesforce (duplicate email found).",
        "error"
      );
    } else if (
      ...
      showStatus(
        'Session expired. Please click "Disconnect" then reconnect to Salesforce.',
        "error"
      );
    } else if (errorMsg.includes("REQUIRED_FIELD_MISSING")) {
      showStatus(
        "⚠️ Missing required field. Please fill in all required information.",
        "error"
      );
    } else if (errorMsg.includes("No such column")) {
      showStatus(
        "⚠️ Field configuration error. Please contact your administrator.",
        "error"
      );
    } else {
