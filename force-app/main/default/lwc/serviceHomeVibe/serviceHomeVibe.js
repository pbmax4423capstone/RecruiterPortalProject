...
export default class ServiceHomeVibe extends NavigationMixin(LightningElement) {
    ...
    if (result.data) {
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          title: "Error",
          message:
            "Failed to load your open cases: " +
    ...
    this.recentItems = [
      {
        ...
        Description:
          "Customer experiencing intermittent network connectivity problems",
        isMockData: true
      },
      {
        Id: "task001",
  ...
  handleRecentItemClick(event) {
    ...
    const item = this.recentItems.find(
  ...
  getObjectApiName(item) {
    ...
    if (item.Type === "Case" || item.IconName === "standard:case") {
      ...
      item.Type === "Contact" ||
      item.IconName === "standard:contact"
    ) {
      return "Contact";
    } else if (
      item.Type === "Account" ||
      item.IconName === "standard:account"
    ) {
      return "Account";
    } else if (
      item.Type === "Opportunity" ||
      item.IconName === "standard:opportunity"
    ) {
      return "Opportunity";
    ...
    try {
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          title: "Error",
          message:
            "Failed to refresh your open cases: " +
  ...
  handleTaskClick(event) {
    ...
    const task = [...this.upcomingTasks, ...this.pastDueTasks].find(
      (t) => t.id === taskId
    );
  ...
  getTaskDescription(task) {
    const descriptions = {
      task001:
        "Follow up with customer regarding Case #12345. Ensure all network connectivity issues have been resolved and document the resolution.",
      task002:
        "Conduct comprehensive review of current security protocols and update documentation based on latest compliance requirements.",
      task003:
        "Prepare agenda and materials for upcoming team meeting. Review open cases and assign priorities for the week.",
      task004:
        "Complete audit of all devices in the network. Document any security vulnerabilities and create remediation plan.",
      task005:
        "Update all technical documentation to reflect recent system changes and new procedures implemented this quarter."
    };
    ...
    try {
      if (event && event.detail) {
        ...
        if (status === "FINISHED") {
          ...
          this.showToast(
            "Success",
            "Device inspection record created successfully!",
            "success"
          );
          ...
          this.showToast(
            "Error",
            "There was an issue with the device inspection flow. Please try again or contact support.",
            "error"
          );
        }
      }
    } catch (error) {
      console.error("Error handling device inspection flow:", error);
      this.showDeviceInspectionModal = false;
      this.showToast(
        "Error",
    ...
    try {
      if (event && event.detail) {
        ...
        if (status === "FINISHED") {
          ...
          this.showToast(
            "Success",
            "Device encryption completed successfully!",
            "success"
          );
        } else if (status === "ERROR") {
          // Handle flow errors gracefully
          ...
          this.showToast(
            "Error",
            "There was an issue with the device encryption flow. Please try again or contact support.",
            "error"
          );
        }
      }
    } catch (error) {
      console.error("Error handling device encryption flow:", error);
      this.showEncryptDeviceModal = false;
      this.showToast(
        "Error",
        "An unexpected error occurred. Please try again.",
    ...
    try {
      if (event && event.detail) {
        ...
        if (status === "FINISHED") {
          ...
          this.showToast(
            "Success",
            "Device offboarding completed successfully!",
            "success"
          );
        } else if (status === "ERROR") {
          // Handle flow errors gracefully
          ...
          this.showToast(
            "Error",
            "There was an issue with the device offboarding flow. Please try again or contact support.",
            "error"
          );
        }
      }
    } catch (error) {
      console.error("Error handling device offboarding flow:", error);
      this.showDeviceOffboardingModal = false;
      this.showToast(
        "Error",
        "An unexpected error occurred. Please try again.",
