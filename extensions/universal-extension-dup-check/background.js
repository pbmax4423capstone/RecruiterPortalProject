...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'logout') {
    chrome.storage.local.remove(['accessToken', 'instanceUrl', 'userId'])
...
async function handleAuthentication() {
  ...
  return new Promise((resolve, reject) => {
    chrome.identity.launchWebAuthFlow(
      ...
      async (redirectUrl) => {
        if (chrome.runtime.lastError) {
          console.error('[Universal SF] Auth flow error:', chrome.runtime.lastError);
...
async function checkAuthStatus() {
  const data = await chrome.storage.local.get(['accessToken', 'instanceUrl', 'userId']);
  
  if (data.accessToken && data.instanceUrl && data.userId) {
    // Verify token is still valid
    try {
      const response = await fetch(`${data.instanceUrl}/services/data/v59.0/sobjects`, {
...
async function findDuplicates(data) {
  const { accessToken, instanceUrl } = await chrome.storage.local.get(['accessToken', 'instanceUrl']);
  ...
  if (email || linkedinUrl) {
    let candidateQuery = 'SELECT Id, Name__c, Email__c, LinkedIn_URL__c FROM Candidate__c WHERE ';
    const conditions = [];
...
async function handleCreateCandidate(data, proceedAnyway = false) {
  const { accessToken, instanceUrl, userId } = await chrome.storage.local.get(['accessToken', 'instanceUrl', 'userId']);
  ...
  if (!contactResponse.ok) {
    ...
    throw new Error(`Failed to create Contact: ${error[0]?.message || 'Unknown error'}`);
  }
  ...
  if (!candidateResponse.ok) {
    ...
    throw new Error(`Failed to create Candidate: ${error[0]?.message || 'Unknown error'}`);
  }
